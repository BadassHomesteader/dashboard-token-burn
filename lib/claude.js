...
const { dayBucket, hourBucket } = require('./daybucket');
...
async function processFile(filePath, dailyMap, sessionsByDate, modelTotals, sessionTotals, byHour) {
    return new Promise((resolve) => {
        ...
        rl.on('line', (line) => {
            ...
            if (byHour) byHour[hourBucket(timestamp)] += input + output + cacheRead + cacheCreation;
...
async function generateTokenUsage(forceRefresh = false) {
    ...
    for (const file of files) {
        await processFile(file, dailyMap, sessionsByDate, modelTotals, sessionTotals, byHour);
    }
